/*Project Name: OpenGL Three Scenario*/
//      Author: SAD BIN SIDDIQUE,
//              MD JANNATUN NAIM,
//              TARIF-UL-HAIDER RABBI,
//              S. F. SHEIKH SAADI,
//              MIR MD. SHAKIB MUNNA
//         Key: Up & Down = Day night switch
//              Left & Right = Speed adjust switch
//              Esc = Exit
//              0 = Cover page
//              1 = First scenario
//              2 = Second scenario
...
// Global Variables
const char* soundFiles[] = {"sound1.wav", "sound2.wav", "sound3.wav"};
int currentSoundIndex = 0; // Tracks the currently playing sound

int currentProject = 1; // Default project
int r=0;
int displayCount =0;
...
void sunmoon(int value) {
    ...
    if (isDay) {
        sunY -= 0.01f; // Move the sun down
        if (sunY <= -1.0f) {
            isDay = false; // Switch to night
            moonY = -1.0f;  //Reset the moon position
        }
    }
    else {
        moonY += 0.01f; // Move the moon up
        if (moonY >= 1.2f) {
            isDay = true; // Switch to day
            sunY = 1.2f;  //Reset the sun position
        }
    }
    glutPostRedisplay();
    glutTimerFunc(100, sunmoon, 0);
}
...
void playSound(int index) {
    if (index >= 0 && index < sizeof(soundFiles) / sizeof(soundFiles[0])) {
        PlaySound(soundFiles[index], NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
    }
}
void stopSound() {
...
int i;
	...
	GLfloat x=0.05f; GLfloat y=-0.9f; GLfloat radius =.02f;
	...
	glEnd();
	GLfloat x1=0.2f; GLfloat y1=-0.9f; GLfloat radius1 =.02f;
	glColor3f(0.15f, 0.15f, 0.15f);
	glBegin(GL_TRIANGLE_FAN);
...
// Window front
  ...
  glVertex2f(-0.58f, -0.84f);
  glVertex2f(-0.535f, -0.84f);
...
int i;
	...
	GLfloat x=-0.65f; GLfloat y=-0.94f; GLfloat radius =.02f;
	int triangleAmount = 200; //# of lines used to draw circle
	...
	glEnd();
	GLfloat x1=-0.5f; GLfloat y1=-0.94f; GLfloat radius1 =.02f;
	glColor3f(0.15f, 0.15f, 0.15f);
	glBegin(GL_TRIANGLE_FAN);
		glVertex2f(x1, y1); // center of circle
		for(i = 0; i <= triangleAmount;i++) {
...
void fountain(){
    ...
    glColor3f(0.34f, 0.40f, 0.45f);
    glVertex2f(0.815f, -0.75f);
    glVertex2f(0.825f, -0.75f);
    glVertex2f(0.825f, -0.58f);
    glVertex2f(0.82f, -0.55f);
...
void waterflow(){
    ...
    glBegin(GL_QUADS); //Top Right Middle
    glVertex2f(0.865f, -0.61f);
    glVertex2f(0.855f, -0.61f);
    glVertex2f(0.855f, -0.68f);
    glVertex2f(0.865f, -0.68f);
    glEnd();
    ...
    glBegin(GL_QUADS); //Top Left Middle
    ...
    glBegin(GL_QUADS); //Top Middle
    glVertex2f(0.835f, -0.61f);
    glVertex2f(0.825f, -0.61f);
    glVertex2f(0.825f, -0.68f);
    ...
    glBegin(GL_QUADS); //Bottom Right Middle
    ...
    glBegin(GL_QUADS);  //Bottom Left Middle
    ...
    glBegin(GL_QUADS);  //Bottom Middle
...
int i;
	...
	GLfloat x=-0.65f; GLfloat y=-0.91f; GLfloat radius =.02f;
	int triangleAmount = 200; //# of lines used to draw circle
	...
	glEnd();
	GLfloat x1=-0.5f; GLfloat y1=-0.91f; GLfloat radius1 =.02f;
	glColor3f(0.15f, 0.15f, 0.15f);
	glBegin(GL_TRIANGLE_FAN);
		glVertex2f(x1, y1); // center of circle
		for(i = 0; i <= triangleAmount;i++) {
...
// Upper
  ...
  glVertex2f(0.17f, -0.81f);
  glVertex2f(0.21f, -0.86f);
  glVertex2f(0.03f, -0.86f);
  glEnd();
// Window back
  glBegin(GL_POLYGON);
  ...
  glVertex2f(0.055f, -0.82f);
  glVertex2f(0.11f, -0.82f);
  glVertex2f(0.11f, -0.86f);
  glVertex2f(0.04f, -0.86f);
  glEnd();
// Window front
  glBegin(GL_POLYGON);
...
int i;
	...
	GLfloat x=0.05f; GLfloat y=-0.92f; GLfloat radius =.02f;
	int triangleAmount = 200; //# of lines used to draw circle
	...
	glEnd();
	GLfloat x1=0.2f; GLfloat y1=-0.92f; GLfloat radius1 =.02f;
	glColor3f(0.15f, 0.15f, 0.15f);
	glBegin(GL_TRIANGLE_FAN);
		glVertex2f(x1, y1); // center of circle
		for(i = 0; i <= triangleAmount;i++) {
...
void displayCover(){
    ...
    renderBitmapString(-0.25f, 0.0f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Name: MD JANNATUN NAIM       ID: 21-45039-2");
    renderBitmapString(-0.25f, 0.2f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Name: MD SAD BIN SIDDIQUE    ID: 21-45005-2");
    renderBitmapString(-0.25f, 0.4f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Name: S. F. SHEIKH SAADI     ID: 22-47793-2");
    renderBitmapString(-0.25f, 0.6f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Name: MIR MD. SHAKIB MUNNA   ID: 22-48059-2");
    renderBitmapString(-0.25f, 0.8f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Name: TARIF UL HAIDER RABBI  ID: 23-51299-1");
    renderBitmapString(-0.4f, 0.9f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "3 IMPORTANT HISTORICAL AND MEMORABLE PLACES OF BANGLADESH");
 glFlush(); // Render now
}
void display3() {
...
int main(int argc, char** argv) {
	...
	glutTimerFunc(0, update1, 0); // FOR CAR
